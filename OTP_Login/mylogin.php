<?php
    ...
    $con = new mysqli('localhost', 'root', '', 'main_register', '3307');
    $sql = "select * from studreg where email = '$email' and password = '$password'";
    $sql2 = "select * from teachregs where email = '$email' and password = '$password'";
    ...
    if($con->connect_error) {
        ...
        if($stmt_result->num_rows > 0) {
            ...
            if($data['password'] === $password) {
                require 'Learner_s Tab.html';
            }
            else {
                     echo '<script>
                                    if(confirm("Invalid E-mail or Password")) document.location = "../OTP_Login/login.html";
                    </script>';
                }
    }
        elseif($stmt_result2->num_rows > 0){
            ...
            if($data['password'] === $password) {
                  header('Location: Educator Tab.html');
                }
                else{
                    echo '<script>
                                    if(confirm("Invalid E-mail or Password")) document.location = "../OTP_Login/login.html";
