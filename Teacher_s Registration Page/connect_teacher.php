...
if (isset($_POST['reg_user'])) {
    ...
    $target = "lic/".basename($_FILES['teachli']['name']);
  	...
  	$cv = $_FILES['res']['name'];
	  $sql = "INSERT INTO teach (tl,res) VALUES ('$cv','$image')";
    ...
    $user_check_query = "SELECT * FROM teach WHERE cntc='$cntc' OR email='$email' LIMIT 1";
    ...
    if (count($errors) == 0) {
        ...
        $query = "INSERT INTO teach (fname, mname, lname, email, cntc, password, tl, res) 
                  VALUES('$fname','$mname','$lname', '$email', '$cntc', '$password', '$image', '$cv')";
        mysqli_query($db, $query);
        move_uploaded_file($_FILES['teachli']['tmp_name'], $target) && move_uploaded_file($_FILES['res']['tmp_name'], $target2);
        $_SESSION['username'] = $email;
        $_SESSION['success'] = "Waitng";
